...
mjml-java targets the **MJML v4** specification. All 33 standard components are implemented:
...
mjml-java is designed for server-side rendering. Each `render()` call creates a lightweight pipeline object that is garbage-collected after use. There is no shared mutable state, so the renderer scales naturally with concurrent requests.
