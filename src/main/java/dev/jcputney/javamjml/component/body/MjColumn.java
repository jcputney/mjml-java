...
public class MjColumn extends BodyComponent {
  ...
  public String render() {
    ...
    // Determine if this is a pixel or percentage width
    ...
    addResponsiveMediaQuery(responsiveClass, widthSpec, isPixelWidth);
    ...
    for (int i = 0; i < children.size(); i++) {
      ...
      if (component instanceof BodyComponent bodyComponent) {
        ...
        if (!tdCssClass.isEmpty()) {
          sb.append(" class=\"").append(tdCssClass).append("\"");
        }
  ...
  private String buildInnerTableStyle() {
    Map<String, String> styles = new LinkedHashMap<>();
    String innerBg = getAttribute("inner-background-color", "");
    if (!innerBg.isEmpty()) {
      styles.put("background-color", innerBg);
    }
  ...
  private void addResponsiveMediaQuery(String className, String widthSpec, boolean isPixelWidth) {
    if (isPixelWidth) {
      globalContext.addMediaQuery(className, widthSpec, "");
    } else {
      globalContext.addMediaQuery(className, widthSpec, "%");
    }
  }
