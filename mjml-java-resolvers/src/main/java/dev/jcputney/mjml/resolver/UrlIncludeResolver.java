...
public final class UrlIncludeResolver implements IncludeResolver {
  ...
  public String resolve(String path, ResolverContext context) {
    ...
    if (isHostname(normalizedHost) && !"localhost".equals(normalizedHost) && allowedHosts.isEmpty()) {
      throw new MjmlIncludeException("Hostname URLs require explicit allowlist configuration: " + host);
    }
    ...
    try {
      HttpRequest request =
        HttpRequest.newBuilder().uri(uri).timeout(readTimeout).GET().build();
    ...
    try {
      ...
      for (InetAddress addr : addresses) {
        if (isBlockedAddress(addr)) {
          throw new MjmlIncludeException("SSRF protection: host resolves to private/local address: " + host);
        }
      }
