...
class ErrorHandlingTest {
    ...
    void malformedMjmlThrowsParseException() {
        assertThrows(MjmlParseException.class, () -> MjmlRenderer.render("<mjml><mj-body><mj-section><mj-column>"));
    }

    @Test
    void nonMjmlRootThrowsParseException() {
        assertThrows(MjmlParseException.class, () -> MjmlRenderer.render("<html><body>Hello</body></html>"));
    }
    ...
    void emptyHeadRenders() {
        String html = MjmlRenderer.render("<mjml><mj-head></mj-head><mj-body></mj-body></mjml>")
                .html();
        assertNotNull(html);
        assertTrue(html.contains("<!doctype html>"));
    }
