...
class BoundaryTest {
  ...
  void inputExactlyAtMaxSizeIsAccepted() {
    ...
    MjmlConfiguration config = MjmlConfiguration.builder().maxInputSize(limit).build();
  ...
  void inputOneCharOverMaxSizeIsRejected() {
    ...
    MjmlValidationException ex =
        assertThrows(
            MjmlValidationException.class, () -> MjmlRenderer.render(MINIMAL_MJML, config));
    assertTrue(
  ...
  void nestingDepthAtLimitIsRejected() {
    ...
    MjmlConfiguration config = MjmlConfiguration.builder().maxNestingDepth(3).build();
  ...
  void normalNestingDepthIsAccepted() {
    ...
    MjmlConfiguration config = MjmlConfiguration.builder().maxNestingDepth(100).build();
  ...
  void maxInputSizeZeroOrNegativeThrowsOnBuild() {
    assertThrows(
        IllegalArgumentException.class, () -> MjmlConfiguration.builder().maxInputSize(0).build());
    assertThrows(
        IllegalArgumentException.class, () -> MjmlConfiguration.builder().maxInputSize(-1).build());
  }
