...
/**
 * Tests that MjText block element detection uses proper tag-boundary matching
 * and does not produce false positives for words containing tag-like substrings.
 */
class MjTextBlockElementTest {
  ...
  void paramTagDoesNotTriggerBlockDetection() {
    ...
    assertTrue(html.contains("inline content"),
        "Content with <param> should still render");
  }
  ...
  void actualParagraphTagTriggerBlockDetection() {
    ...
    MjmlRenderResult result = MjmlRenderer.render(mjml);
    String html = result.html();
    assertTrue(html.contains("<p>paragraph content</p>"),
        "Block element <p> should be preserved");
  }
  ...
  void pictureTagDoesNotTriggerBlockDetection() {
    ...
    assertTrue(result.html().contains("text"),
        "Content with <picture> should still render");
  }
