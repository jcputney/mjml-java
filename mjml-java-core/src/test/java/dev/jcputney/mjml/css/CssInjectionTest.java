...
class CssInjectionTest {
  ...
  void styleContentWithCssInjectionIsPreserved() {
    ...
    assertTrue(html.contains("expression(alert(1))"), "CSS content should be preserved within the style block");
  ...
  void stylContentWithCurlyBracesAndAtRules() {
    ...
    String html = MjmlRenderer.render(mjml).html();
    assertNotNull(html);
    assertTrue(html.contains("@media only screen"), "Media query should be preserved in style block");
