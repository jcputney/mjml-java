...
class CircularIncludeTest {
  ...
  void selfIncludeDetected() {
    ...
    assertThrows(
      MjmlException.class, () -> MjmlRenderer.render(mjml, config), "Self-referential include should throw");
  }
  ...
  void deepLinearChainSucceeds() {
    ...
    for (int i = 0; i < 10; i++) {
      resolver.put(
        "level" + i + ".mjml",
        "<mj-section><mj-column><mj-include path=\"level"
          + (i + 1)
          + ".mjml\" /></mj-column></mj-section>");
    }
    resolver.put(
      "level10.mjml", "<mj-section><mj-column><mj-text>Deep leaf content</mj-text></mj-column></mj-section>");
    ...
    assertNotNull(html);
    assertTrue(html.contains("Deep leaf content"), "Non-circular deep chain should render successfully");
  }
  ...
  void diamondIncludePatternSucceeds() {
    ...
    MapIncludeResolver resolver =
      new MapIncludeResolver().put("b.mjml", """
        <mj-section>
          <mj-column>
  ...
  void justBelowDepthLimitSucceeds() {
    ...
    resolver.put("level5.mjml", "<mj-section><mj-column><mj-text>Shallow leaf</mj-text></mj-column></mj-section>");
    ...
    String html = MjmlRenderer.render(mjml, config).html();
    assertNotNull(html);
    assertTrue(html.contains("Shallow leaf"), "Shallow chain should succeed without hitting depth limit");
  }
  ...
  void circularHtmlIncludeDoesNotCycleDetect() {
    ...
    MapIncludeResolver resolver = new MapIncludeResolver().put("banner.html", "<div class=\"banner\">Banner</div>");
