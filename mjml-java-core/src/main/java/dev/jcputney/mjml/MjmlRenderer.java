 ...
 *
 * <p><strong>Thread safety:</strong> Both the static {@code render()} methods and the instance
 * API are thread-safe. Each render call creates its own
 * {@link dev.jcputney.mjml.context.GlobalContext}, so concurrent calls do not share mutable
 * state. The provided {@link MjmlConfiguration} is immutable and safe to share.</p>
 *
 * <h2>Usage</h2>
 * <pre>{@code
...
public final class MjmlRenderer {
   ...
   * The instance caches and reuses the internal {@link RenderPipeline} and
   * {@link dev.jcputney.mjml.component.ComponentRegistry}, making it significantly
   * faster for repeated rendering than the static convenience methods.
   *
   * <p>This is the <strong>preferred API</strong> when rendering multiple templates
  ...
  public MjmlRenderResult renderTemplate(Path mjmlFile) {
    try {
      ...
      if (configuration.getIncludeResolver() == null) {
        ...
        MjmlConfiguration fileConfig = configuration.toBuilder()
  ...
  /**
   * Renders an MJML template to HTML with a specific include resolver,
   * overriding the instance's configured resolver for this call only.
   *
   * @param mjml     the MJML source string
  ...
  public MjmlRenderResult renderTemplate(String mjml, IncludeResolver resolver) {
    MjmlConfiguration resolverConfig = configuration.toBuilder()
   ...
   *
   * <p>For repeated rendering, prefer the instance API via {@link #create(MjmlConfiguration)}.</p>
   *
   * @param mjml the MJML source string
   * @return the render result containing HTML and metadata
   * @throws MjmlException if parsing or rendering fails
