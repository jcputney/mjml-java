...
public final class IncludeProcessor {
  ...
  private void processNode(MjmlNode node, List<String> includeChain, Set<String> visitedPaths, int depth) {
    if (node == null) {
      return;
    }
  ...
  private void resolveInclude(MjmlNode includeNode, List<String> includeChain, Set<String> visitedPaths, int depth) {
    ...
    if (maxInputSize > 0 && content.length() > maxInputSize) {
      throw new MjmlValidationException("Included content from '"
        + path
  ...
  private void resolveAsMjml(
    MjmlNode includeNode, String mjmlContent, List<String> includeChain, Set<String> visitedPaths, int depth) {
