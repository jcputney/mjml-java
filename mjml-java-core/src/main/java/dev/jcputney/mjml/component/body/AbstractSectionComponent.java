...
public abstract class AbstractSectionComponent extends BodyComponent {
  ...
  /**
   * Builds the style for the inner td element. Subclasses can override
   * {@link #addInnerTdBorderStyles(Map)} and {@link #addInnerTdExtraStyles(Map)}
   * to customize border handling and add extra properties.
   */
  ...
  /**
   * Adds border styles to the inner td style map. Default implementation adds only
   * the "border" attribute if non-empty and not "none". Subclasses can override
   * to use the full addBorderStyles() helper.
   */
   ...
   * Both components follow the same MSO table → VML rect → div/table → inner content → close
   * structure; subclasses provide the VML rect string, inner content, and optional css-class
   * on the outer div.
   *
  ...
  protected String renderNormalScaffold(String vmlRect, String innerContent, String outerDivClass) {
    ...
    if (hasBgUrl) {
      ...
      sb.append("    </div>\n");
      sb.append("    ").append(MsoHelper.conditionalStart())
          .append("</v:textbox></v:rect>").append(MsoHelper.msoTableClosing())
