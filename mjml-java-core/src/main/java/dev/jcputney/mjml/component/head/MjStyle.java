...
/**
 * Adds CSS styles from {@code <mj-style>} content to the rendered HTML. Supports {@code
 * inline="inline"} attribute for styles that should be inlined.
 *
 * <p><strong>Security note:</strong> {@code mj-style} passes CSS content through to the rendered
 * HTML with minimal filtering (only {@code </style>} tag injection is stripped). Untrusted CSS can
 * enable data exfiltration via {@code url()} references or attribute selectors that leak content.
 * When processing untrusted MJML input, configure a {@link dev.jcputney.mjml.ContentSanitizer} to
 * validate or strip dangerous CSS constructs before rendering.
 */
