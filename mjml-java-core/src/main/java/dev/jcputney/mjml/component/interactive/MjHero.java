...
public class MjHero extends BodyComponent {
  ...
  public MjHero(MjmlNode node, GlobalContext globalContext, RenderContext renderContext, ComponentRegistry registry) {
  ...
  /**
   * Renders a fixed-height hero. The content td has an explicit height derived from the declared height minus vertical
   * padding.
   */
  private String renderFixedHeight() {
    ...
    return renderHero(height, innerHeight, null);
  }

  /**
   * Renders a fluid-height hero. Height adapts to content; spacer tds with padding-bottom percentage maintain the
  ...
  private String renderFluidHeight() {
    ...
    return renderHero(bgHeight, 0, spacerPaddingPct);
  }
  ...
  private void appendVmlImage(HtmlBuilder html, String backgroundUrl, String vImageHeight, int containerWidth) {
    if (!backgroundUrl.isEmpty()) {
      String heightStyle = vImageHeight.isEmpty() ? "" : "height:" + escapeAttr(vImageHeight) + ";";
      html.rawVerbatim("<v:image style=\"border:0;"
        + heightStyle
  ...
  private void appendHeroContent(HtmlBuilder html) {
    ...
    String tableAttrs = attrs("border", "0", "cellpadding", "0", "cellspacing", "0",
