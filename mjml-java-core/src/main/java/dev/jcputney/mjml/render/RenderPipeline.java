...
public final class RenderPipeline {
  ...
  private static final Pattern MSO_SECTION_MERGE = Pattern.compile(
    "<!--\\[if mso \\| IE]></td></tr></table><!\\[endif]-->\\s+" + "<!--\\[if mso \\| IE]><table ");
  private static final Pattern MSO_VML_MERGE =
    Pattern.compile("<!--\\[if mso \\| IE]></v:textbox></v:rect></td></tr></table><!\\[endif]-->\\s+"
      + "<!--\\[if mso \\| IE]><table ");
  ...
  private static String mergeMsoSectionTransitions(String html) {
    html = MSO_SECTION_MERGE.matcher(html).replaceAll("<!--[if mso | IE]></td></tr></table><table ");
    html = MSO_VML_MERGE
      .matcher(html)
  ...
  public MjmlRenderResult render(String mjmlSource) {
    ...
    try {
      ...
      return new MjmlRenderResult(
        html,
        globalContext.metadata().getTitle(),
        globalContext.metadata().getPreviewText());
  ...
  private ComponentRegistry createRegistry() {
    ...
    reg.register("mj-accordion-element", (node, ctx, rctx) -> new MjAccordionElement(node, ctx, rctx, reg));
    ...
    for (Map.Entry<String, dev.jcputney.mjml.component.ComponentFactory> entry : configuration.getCustomComponents()
      .entrySet()) {
      reg.register(entry.getKey(), entry.getValue());
    }

    // Register custom container components (with registry access)
    for (Map.Entry<String, ContainerComponentFactory> entry : configuration.getCustomContainerComponents().entrySet()) {
      reg.register(entry.getKey(), (node, ctx, rctx) -> entry.getValue().create(node, ctx, rctx, reg));
    }
